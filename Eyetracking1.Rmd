...
library(pacman)
p_load(data.table, lmerTest, dplyr, groupdata2, stringr, MuMIn, cvms, ggplot2)
...
CrossVal = function(num_folds, dataset, mdl_string, ID_col = NULL, CAT_col = NULL, glmer = T) {
...
m1 = lmer(Amplitude ~ SearchType + Saccade +  (1 + Saccade + SearchType|ParticipantID), subset(df_sac1, Task == "VisualSearch"), REML = F)
summary(m1)

m1 = lmer(Duration ~ SearchType + (1 + SearchType|ParticipantID), subset(df_fix1, Task == "VisualSearch"), REML = F)
summary(m1)

m3 = glmer(Duration ~ SearchType + (1 + SearchType|ParticipantID), subset(df_fix1, Task == "VisualSearch"), family = gaussian(link = "log"), 
                control = #adding an optimizer
                  glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))

mdl_list = c("Duration ~ SearchType + (1 + SearchType|ParticipantID)", 
             "Duration ~ SearchType + Trial + (1 + Trial + SearchType|ParticipantID)", 
             "Duration ~ SearchType*Trial + (1 + Trial + SearchType|ParticipantID)" 
             )


VS = subset(df_fix1, Task == "VisualSearch")

#setting vectors to be the right types
VS$Task = as.factor(as.character(VS$Task))
VS$Trial = as.integer(VS$Trial)
VS$ParticipantID = droplevels(VS$ParticipantID)
VS$SearchType = as.factor(VS$SearchType)

perf_df = CrossVal(num_folds = 3, dataset = VS, mdl_string = mdl_list[1], ID_col = "ParticipantID", CAT_col = "SearchType", glmer = T)


#Using ludvigs package (doesn't seem to work)
...
#Heatmap
ggplot(subset(df_fix1, Task=='VisualSearch' & ParticipantID=='2_2_f2' & Trial==1), aes(x = PositionX, y = PositionY)) +
  xlim(0,1920) +
  ylim(0, 1080) +
...
ggplot(subset(df_fix1, Task=='VisualSearch' & ParticipantID=='2_2_f2' & Trial==6), aes(x = PositionX, y = PositionY)) +
  xlim(0,1920) +
  ylim(0, 1080) +
